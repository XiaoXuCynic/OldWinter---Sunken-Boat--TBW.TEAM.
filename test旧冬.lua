...
-- 重要修复：先创建 Tabs 再使用它
local Tabs = {
    Main = Window:Section({ Title = "主要功能/脚本", Opened = true }),
...
-- 创建选项卡 - 使用字符串格式（最简单可靠）
local TabHandles = {
    Elements = Tabs.Main:Tab("功能", "layout-grid"),
    Appearance = Tabs.Settings:Tab("修改背景", "brush"),
    Config = Tabs.Utilities:Tab("配置", "settings"),
    LiJian = Tabs.FE:Tab("FE脚本", "play"),
    XiaoXu = Tabs.Script:Tab("脚本工具", "crown")
}
...
safeCreate(function()
    ElementsSection = TabHandles.Elements:Section({
        Title = "通用功能",
        Icon = "crown",
    })
end)
...
local themeDropdown
safeCreate(function()
    themeDropdown = TabHandles.Appearance:Dropdown({
        Title = "改颜色",
        Values = themes,
        Value = "Dark",
...
safeCreate(function()
    transparencySlider = TabHandles.Appearance:Slider({
        Title = "背景颜色",
        Min = 0,
        Max = 1,
        Default = 0.2,
...
local ThemeToggle
safeCreate(function()
    ThemeToggle = TabHandles.Appearance:Toggle({
        Title = "启用深色模式",
        Desc = "使用深色配色方案",
        Value = true,
        Callback = function(state)
...
local LiJianSection
safeCreate(function()
    LiJianSection = TabHandles.LiJian:Section({
        Title = "FE脚本",
        Icon = "crown",
    })
end)
...
local XiaoXuSection
safeCreate(function()
    XiaoXuSection = TabHandles.XiaoXu:Section({
        Title = "脚本工具",
        Icon = "crown",
    })
end)
